...
impl PublicKey {
    ...
    /// Obtain a public key from a secret key
    ///
    /// Compute a public key from a secret key previously generated by [`KeyPair::generate`]
    /// by doing `scalarmult_base` with the secret key.
    ///
    /// # Returns
    ///
    ...
    /// # Errors
    ///
    /// Returns an error if compute the public key from the secret key
    ///
    /// # Security Considerations
    ///
    ...
    /// use sodium::crypto_box::curve25519xsalsa20poly1305::{KeyPair, PublicKey};
    ...
    /// let extracted_public_key = PublicKey::from_secret_key(&secret_key).unwrap();
    ///
    ...
    pub fn from_secret_key(secret_key: &SecretKey) -> Result<Self> {
        crate::crypto_scalarmult::curve25519::scalarmult_base(secret_key.as_bytes()).map(Self::from)
    }
}
...
impl PrecomputedKey {
    ...
    ///
    /// # Returns
    /// * `&[u8]` - A reference to the precomputed key bytes
    pub fn as_bytes(&self) -> &[u8] {
        &self.0
    }
}
...
mod tests {
    ...
    fn test_precomputedkey_traits() {
        ...
        // Test TryFrom<&[u8]>
        let key = PrecomputedKey::try_from(bytes).unwrap();
        assert_eq!(key.as_bytes(), bytes);
        ...
        let mut array = [0u8; BEFORENMBYTES];
        array.copy_from_slice(bytes);
        let key2 = PrecomputedKey::from(array);
        assert_eq!(key2.as_bytes(), &array);
